    ...
    px-6 py-4 flex items-center justify-between z-40 shadow-lg shadow-slate-200/50'>
            ...
            <div className='flex items-center gap-3 mx-auto sm:mx-0'>
                ...
                <button onClick={onToggleAudio}
                    className={`p-3.5 rounded-2xl transition-all cursor-pointer border ${audioEnabled ? "bg-slate-100 hover:bg-slate-200 text-slate-800 border-slate-300 shadow-xs"
                ...
                <button onClick={onToggleVideo}
                    className={`p-3.5 rounded-2xl transition-all cursor-pointer border ${videoEnabled ? "bg-slate-100 hover:bg-slate-200 text-slate-800 border-slate-300 shadow-xs"
                    : "bg-rose-50 hover:bg-rose-100 text-rose-600 border-rose-200 shadow-xs"
                    }`}
                ...
                <button onClick={onToggleChat}
                    ...
                    {unreadCount > 0 && !isChatOpen && (
                        ...
                        font-bold w-5 h-5 rounded-full flex items-center justify-center border-2
                ...
                <button onClick={onToggleParticipants}
                    ...
                    <span className='absolute -top-1 -right-1 bg-slate-200 text-slate-800
                    text-[10px] font-bold px-1.5 py-0.5 rounded-full border border-slate-300'>
                        {participantCount}
                    </span>
                </button>
